...
#fxtrain=tf.reshape(xtrain,(-1,17*129*3))
fxval=tf.reshape(xval,(-1,17*129*3))
fxtest=tf.reshape(xtest,(-1,17*129*3))

input_shape = xtrain.shape[1:]
print("Shape of training data: {}".format(xtrain.shape))
print("Shape of training tensor: {}".format(fxtrain.shape))

# Define and train autoencoder
# ==================================================
model = DenseAutoencoder(input_shape, activation_function = "elu", latent_dim = latent_dim, learning_rate = learning_rate)
autoencoder = model.autoencoder()

early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
fitted_model = autoencoder.fit(fxtrain, fxtrain, epochs=number_of_epochs, batch_size=batch_size,
                        validation_data=(fxval,fxval), callbacks=[early_stopping])

autoencoder.save('dense_100_ae.h5')
# autoencoder = tf.keras.models.load_model('dense_100_ae.h5')

# Create reconstructions
# ==================================================
val_recon = autoencoder.predict(fxval)
val_recon = tf.reshape(val_recon,(-1,17,129,3))

test_recon = autoencoder.predict(fxtest)
test_recon = tf.reshape(test_recon,(-1,17,129,3))
...
plt.legend()
plt.savefig("dense_100_ae_train_val_loss.png")
...
val_features = encoder.predict(fxval)
test_features = encoder.predict(fxtest)
...
tf.io.write_file('dense_100_ae_features.tfrecord', serialized_tensor)
...
joblib.dump(gmm, 'dense_100_ae_gmm.pkl')
...
plt.title('ROC Curve')
plt.legend()
plt.savefig("dense_100_ae_roc_auc_curve")
...
plt.savefig("dense_100_ae_cm.png")
plt.show()
