...
# Simple autoencoder based on convolutional layers
    ...
    def __init__(self, config, xtrain, xval, latent_dim):
        self.config = config

        self.input_shape = xtrain.shape[1:]
        self.latent_dim = latent_dim
        
    def model(self):

        # Encoder
        encoder_input = tf.keras.Input(shape=self.input_shape) # (17,129,1)
        x = tf.keras.layers.ZeroPadding2D(padding=((1, 0), (0, 0)))(encoder_input) # (18,129,1)
        
        x = tf.keras.layers.Conv2D(32, (3, 3), activation='sigmoid', padding='same')(x)  # Output: (18, 129, 32)
        x = tf.keras.layers.MaxPooling2D((3, 3), padding='same')(x)  # Output: (6, 43, 32)
        x = tf.keras.layers.Conv2D(64, (3, 3), activation='sigmoid', padding='same')(x)  # Output: (6, 43, 64)
        xs = np.shape(x)
        x = tf.keras.layers.Flatten()(x)
        encoder_output = tf.keras.layers.Dense(self.latent_dim, activation="sigmoid")(x)
        encoder = tf.keras.Model(encoder_input, encoder_output, name="encoder")

        # Decoder
        x = tf.keras.layers.Dense(units=xs[1]*xs[2]*xs[3], activation="sigmoid")(encoder_output)
        x = tf.keras.layers.Reshape((xs[1], xs[2], xs[3]))(x)
        x = tf.keras.layers.Conv2DTranspose(64, (3, 3), activation='sigmoid', padding='same')(x)  # Output: (9, 66, 64)
        x = tf.keras.layers.Conv2DTranspose(32, (3, 3), activation='sigmoid', padding='same')(x)
        x = tf.keras.layers.UpSampling2D((3, 3))(x)
        x = tf.keras.layers.Conv2D(3, (3, 3), activation='sigmoid', padding='same')(x)  # Output: (17, 129, 1)
        ...
        self.summary=autoencoder.summary()
